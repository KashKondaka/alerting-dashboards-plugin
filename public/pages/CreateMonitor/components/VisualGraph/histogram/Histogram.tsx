...
import { Chart as IChart } from '../utils/ChartTypes';
...
export const DiscoverHistogram: React.FC<DiscoverHistogramProps> = ({
  ...
  useEffect(() => {
    if (services?.theme?.chartsTheme$ && services?.theme?.chartsBaseTheme$) {
  ...
  const renderBarTooltip = useCallback(
    (xInterval: number, domainStart: number, domainEnd: number) => (
      ...
      const partialDataText = i18n.translate(
        'alerting.discover.histogram.partialData.bucketTooltipText',
        {
          defaultMessage:
      ...
      if (headerDataValue < domainStart || headerDataValue + xInterval > domainEnd) {
        return (
          <React.Fragment>
            <EuiFlexGroup
              alignItems="center"
              className="alertingHistogram__header--partial"
              data-test-subj="alertingHistogramHeader"
              responsive={false}
              gutterSize="xs"
            >
  ...
  const chartsBaseTheme = services.theme?.chartsDefaultBaseTheme || 'light';
